...
export class ConfidentialTransactionBuilder extends TransactionBuilder {
     ...
     */
    setWitness(input: Prevout, sig: ISignature): boolean {
     ...
     */
    puzzleDestroyWitness(bidPrevouts: BlindPrevout[], seller: ISignature[], buyer: ISignature[]): boolean {
        this.tx.inputs.forEach((input, i) => {
            if (i === 0) { // Sellers output
                input.setWitness([
                    ...
                    new Buffer('00', 'hex'),
                    new Buffer(bidPrevouts[0]._redeemScript, 'hex'),
                ]);
            } else if (i === 1) {
                input.setWitness([
                    ...
                    new Buffer('00', 'hex'),
                    new Buffer(bidPrevouts[1]._redeemScript, 'hex'),
                ]);
            }
        });
     ...
     * @param bidPrevouts The previous output from the bid transaction (buyer or seller)
     * @param party The signatures of the buyer or seller
     * @param secret The secret revealed by the buyer
     */
    puzzleReleaseWitness(bidPrevout: BlindPrevout, party: ISignature, secret: string): boolean {
        const input = this.tx.inputs.find((input) => (input.outputIndex === bidPrevout.vout));
    ...
    getCommitmentForVout(vout: number) {
        return this.tx.outputs[vout].valueCommitment.toString('hex');
    };

    getPubKeyScriptForVout(vout: number) {
...
export function buildBidTxScript(addressFrom: CryptoAddress, addressTo: CryptoAddress, hashedSecret: string, secondsToLock: number): any {
    ...
    let redeemScript = bitcore.Script('OP_IF OP_SIZE 0x01 0x20 OP_EQUALVERIFY OP_SHA256')
        ...
        .add(getSerializedInteger(this.getExpectedSequence(secondsToLock))) // Sequence
 ...
 */
export function getExpectedSequence(seconds: number) {
...
export function buildDestroyTxScript(): any {
    // create a multisig redeemScript
    let redeemScript = bitcore.Script('OP_RETURN')
